...
class GitRepo:
    ...
    def __init__(self, repo_id: int, name: str, url: str, branch: str = "main", 
        ...
        self.repo_path = os.path.join(self.work_dir, f"{self.repo_id}_{self.name.replace(' ', '_')}")
    ...
    def pull(self) -> bool:
        ...
        try:
            ...
            if not os.path.exists(self.repo_path):
                logger.warning(f"Repository {self.name} not found at {self.repo_path}, cloning instead")
                return self.clone()
    ...
    def get_commits_since(self, since_commit: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"Getting commits since {since_commit} for repository {self.name}")
            result = subprocess.run(
                ["git", "log", f"{since_commit}..HEAD", "--format=%H%n%an%n%ae%n%at%n%s", "--reverse"],
                cwd=self.repo_path, check=True, capture_output=True, text=True
            )
            ...
            for block in commit_blocks:
                ...
                if len(lines) < 5:
                    logger.warning(f"Unexpected git log output format in block: {block}")
                    continue
            ...
            logger.error(f"Failed to get commits since {since_commit} for repository {self.name}: {e}")
            logger.error(f"Stdout: {e.stdout}")
            logger.error(f"Stderr: {e.stderr}")
            return []
        except Exception as e:
            logger.error(f"Error getting commits since {since_commit} for repository {self.name}: {e}")
            return []
    ...
    def checkout_commit(self, commit_hash: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to checkout commit {commit_hash} for repository {self.name}: {e}")
            logger.error(f"Stdout: {e.stdout}")
            logger.error(f"Stderr: {e.stderr}")
            return False
        except Exception as e:
            logger.error(f"Error checking out commit {commit_hash} for repository {self.name}: {e}")
            return False
...
class GitTracker:
    ...
    def add_repository(self, repo_id: int, name: str, url: str, branch: str = "main") -> bool:
    ...
    def _get_latest_commit_hash(self, repo_id: int) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Error getting latest commit hash for repository {repo_id}: {e}")
            return None
